...
public interface EnvironmentsService {
    ...
    System getFullSystemById(UUID systemId);

    List<Connection> getConnectionsSystemById(UUID systemId);

    System getFullSystemByName(@Nonnull UUID projectId, @Nonnull UUID environmentId, @Nonnull String systemName);

    LazySystem getLazySystemByName(@Nonnull UUID projectId, @Nonnull UUID environmentId, @Nonnull String systemName);

    LazySystem getLazySystemById(@Nonnull UUID systemId);
    ...
    List<LazySystem> getLazySystems(@Nonnull UUID environmentId, @Nonnull String defaultSystem);

    List<LazySystem> getLazySystemsByProjectIdWithConnections(UUID projectId);

    List<LazyEnvironment> getLazyEnvironmentsShort(@Nonnull UUID projectId);
